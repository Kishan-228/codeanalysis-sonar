...
def deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account_nm, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, core_account_name, account_number):
...
def main(env, app_name, bb_repo_url, bb_branch_name, bb_repo_name, local_tf_base, local_devops_base,local_bb_base, workspace_identifier, override_workspace_details, skip_deploy, terraform_deployment, exec_code_base_type, dry_run, cloud_prov, platform_nm, account_number, upgrade_param=""):
    ...
    if cloud_prov == "aws":
        ...
        if(str(workspace_identifier).__contains__("_CORE_")):
            ...
            deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, acoount_number, core_account_name='AEDL' )
                
        else:
            core_noncore_param = "noncore"
            deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account_nm, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, account_number, core_account_name='' )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--platform_nm', metavar='<Platform name>', help='Platform is where the deployments are going to happen; Valid values are DPDF|AEDL',
                        type=str,
                        default="aedl")
    parser.add_argument('--account_number', metavar='<account number>', help='account numbers to differ the accounts',
                        type=str,
                        default="")
