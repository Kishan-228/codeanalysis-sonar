...
def deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account_nm, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, account_name, core_account_name):
...
def main(env, app_name, bb_repo_url, bb_branch_name, bb_repo_name, local_tf_base, local_devops_base,local_bb_base, workspace_identifier, override_workspace_details, skip_deploy, terraform_deployment, exec_code_base_type, dry_run, cloud_prov, platform_nm, account_name, upgrade_param=""):
    ...
    if cloud_prov == "aws":
        ...
        if(str(workspace_identifier).__contains__("_CORE_")):
            ...
            service_name = bb_repo_name.split('_')[-1].lower()
            logger.info(f"type of service: {service_name}")
            if service_name in ["dvpssb", "testing"]:
                
                print("Parallel deployment")

                if  account_name in yaml_core_group_list: 
                    #if (account == account_name):
                        print(f"account name : {account_name} matched")
                        yaml_core_account_app_list = yaml.full_load(open(local_devops_base + '/resources/ref_config/' + cloud_prov.lower() + '/' + platform_nm.lower() + '/' + 'core_app_mapping.yaml'))[account_name]
                        print(f"check1: {yaml_core_account_app_list}")
                        print(f"check2:{app_name}")
                        # Overriding app_name as per the core app mapping instead of taking the one got in main
                        if app_name in yaml_core_account_app_list + ["EDL"]: 
                            #if (app_name == account_name):
                                print(f"app name : {app_name} matched")
                                deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, account_name, core_account_name='AEDL' )
                                
                            #else:
                                #print(f"app name : {app_name} unmatched, skipping")
                                #break(fail the pipeline)
                        else:
                            print(f"app name : {app_name} unmatched, skipping")
                            print("fail the pipeline")
                        
                else:
                    print(f"account name : {account_name} unmatched, skipping")
            else:
                print("Sequential deployment")
                for account in yaml_core_group_list:
                    ...
                    for app_name in yaml_core_account_app_list:
                        deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, account_name, core_account_name='AEDL')
        else:
            core_noncore_param = "noncore"
            
            deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account_nm, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, account_name, core_account_name='' )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--platform_nm', metavar='<Platform name>', help='Platform is where the deployments are going to happen; Valid values are DPDF|AEDL',
                        type=str,
                        default="aedl")
    parser.add_argument('--account_name', metavar='<account number>', help='account numbers to differ the accounts',
                        type=str,
                        default="")
