...
def deploy_app(logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account_nm, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, core_account_name):
            ...
            with ThreadPoolExecutor() as executor:
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        logger.error(f"Deployment generated an exception: {e}")

        else:
            core_noncore_param = "noncore"
            with ThreadPoolExecutor() as executor:
                future = executor.submit(deploy_app, logger, bb_repo_url, bb_branch_name, bb_repo_name, upgrade_param, dry_run, local_bb_base, local_devops_base, cloud_prov, platform_nm, core_noncore_param, env, account_nm, app_name, workspace_identifier, override_workspace_details, exec_code_base_type, build_config_dl, core_account_name='')
                try:
                    future.result()
                except Exception as e:
                    logger.error(f"Deployment generated an exception: {e}")
